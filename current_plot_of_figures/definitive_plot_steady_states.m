...
% labels_for_boxplot=[{'1x'};{'1/2x'};{'1/3x'}];
% monomers=[{'sGCTA'};{'sGCTA_1_2'};{'sGCTA_1_3'}; {'lGCTA'};{'lGCTA_1_2'};{'lGCTA_1_3'}]
% monomers=[{'sGCTA'};{'lGCTA'};{'lcGCTA'}];
% labels_for_boxplot=[{'short\nrigid'};{'long\nflexible'};{'long\nrigid'}];
% color per group=[0.749, 0.749, 0.749;0,0.524,0.612;0.929,0.490,0.191];
file_name='final.mat';
...
set(pp,'LineWidth',2) % change widith of lines (all lines)
set(pp(7,:),'MarkerSize',5) % change size of the marker
set(mm,'YTick',[0:0.2:1]);
set(get(mm,'XAxis'),'FontWeight','bold'); % change the Font weight of the xTick
set(get(mm,'YAxis'),'FontWeight','bold'); % change the Font weight of the yTick
...
for j=1:length(index_of_the_boxes)
    patch(get(property_element_boxplot(index_of_the_boxes(j)),'XData'),get(property_element_boxplot(index_of_the_boxes(j)),'YData'),colors_per_label(j,:)*0,'FaceAlpha',.5);
end
